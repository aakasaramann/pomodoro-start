...
def start_timer():
    ...
    work_count_sec = WORK_MIN*60
    break_count_sec = SHORT_BREAK_MIN*60
    long_break_count_sec = LONG_BREAK_MIN*60

    if reps % 8 == 0:
        count_down(long_break_count_sec)
        timer_label.config(text="Break", fg=RED)
    elif reps % 2 == 0:
        count_down(break_count_sec)
        timer_label.config(text="Break", fg=PINK)
    else:
        count_down(work_count_sec)
